...
class CacheManager:
    ...
    """
    def __init__(self):
        self.db_path = current_directory / "cache.db"
        conn = self._get_conn()
        cursor = conn.cursor()
    ...
    def check_mojang_cache(self, search_term: str, time_between_cache: int = 360):
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            results = cursor.execute("SELECT timestamp FROM mojang_cache WHERE uuid = ? OR LOWER(username) = ?", (search_term.lower(), search_term.lower(),)).fetchall()
            try:
    ...
    def get_data_from_mojang_cache(self, search_term: str):
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM mojang_cache WHERE uuid = ? OR LOWER(username) = ?", (search_term.lower(), search_term.lower(),))
    ...
    def add_hypixel_cache(self, uuid, hypixel_data: dict):
        with self._get_conn() as conn:
            ...
            if hypixel_data["status"] != "success":
                ...
                return
            cursor.execute(
                """INSERT OR REPLACE INTO hypixel_player_cache (uuid, first_login, last_login, rank, guild_id, timestamp)
                VALUES (?, ?, ?, ?, ?, strftime('%s', 'now'))""", 
                (uuid, hypixel_data["first_login"], hypixel_data["last_login"],hypixel_data["player_rank"], hypixel_data["guild_id"])
            )
    ...
    def check_hypixel_player_cache(self, uuid: str, time_between_cache: int = 360):
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            results = cursor.execute("SELECT timestamp FROM hypixel_player_cache WHERE uuid = ?", (uuid,)).fetchall()
            try:
                last_timestamp = results[0][0]
                if self._is_cache_valid(last_timestamp, time_between_cache):
    ...
    def get_hypixel_player_cache(self, uuid: str):
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM hypixel_player_cache WHERE uuid = ?", (uuid,))
            results = cursor.fetchone()
    ...
    def check_hypixel_guild_cache(self, guild_id: str, time_between_cache: int = 720):
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            results = cursor.execute("SELECT timestamp FROM hypixel_guild_cache WHERE guild_id = ?", (guild_id,)).fetchall()
            try:
                last_timestamp = results[0][0]
                if self._is_cache_valid(last_timestamp, time_between_cache):
    ...
    def get_hypixel_guild_cache(self, guild_id: str):
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM hypixel_guild_cache WHERE guild_id = ?", (guild_id,))
            results = cursor.fetchone()
    ...
    def get_usernames_for_uuids_from_cache(self, uuids: list[str]) -> dict:
        ...
        with self._get_conn() as conn:
            ...
            query = f"SELECT uuid, username, skin_showcase_b64 FROM mojang_cache WHERE uuid IN ({placeholders})"
            
            try:
                rows = cursor.execute(query, uuids).fetchall()
                # This is a dictionary comprehension, a concise way to build a dict from a list.
                return {uuid: {"username": username, "skin_showcase_b64": skin_b64} for uuid, username, skin_b64 in rows}
    ...
    def clear_cache(self):
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM hypixel_guild_cache")
            cursor.execute("DELETE FROM hypixel_player_cache")
            cursor.execute("DELETE FROM mojang_cache")
            conn.commit()

            cursor.execute("VACUUM") # clear extra space
            conn.commit()
